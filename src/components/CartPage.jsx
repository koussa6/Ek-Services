...
const CartPage = () => {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden py-16 sm:px-6 lg:px-8 bg-gradient-to-br from-[#1a120b] via-[#2a1e14] to-[#3e2b1d]">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-center mb-12 animate-fade-in-down">
          <span className="font-dancingscript block text-5xl sm:text-6xl md:text-7xl mb-2 bg-gradient-to-r from-amber-400 bg-clip-text text-transparent">
            Your Cart
          </span>
        </h1>
        {cartItems.length === 0 ? (
          <div className="text-center animate-fade-in ">
            ...
            <Link
              to="/menu"
              className="transition-all duration-300 text-amber-100 inline-flex items-center gap-2 hover:gap-3 hover:bg-amber-800/50 bg-amber-900/40 px-6 py-2 rounded-full font-cinzel text-uppercase"
            >
              Browser All Items
            </Link>
          </div>
    ...
    transform hover:-translate-y-1 animate-fade-in"
        ...
        rounded-lg transition-transform duration-300"
                    onClick={() =>
                      setSelectedImage(buildImageUrl(item.imgUrl || item.image))
                    }
                  >
                    <img
                      src={buildImageUrl(item.imgUrl || item.image)}
                      alt={item.name}
      ...
      {selectedImage && (
        ...
        >
          <div className="relative max-w-full max-h-full">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-1 right-1 bg-amber-900/80 rounded-full p-2 text-black hover:bg-amber-800/90 transition duration-200 active:scale-95"
            >
