...
const AboutHome = () => {
  return (
    <div className="min-h-screen bg-gradient-to-r from-[#0f0a0a] via-[#1a1212] to-[#2a1e1e] text-white py-10 sm:py-20 relative overflow-hidden">
      <div className="absolute left-0 top-0 w-full h-full opacity-10 pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-20 w-80 h-80 bg-amber-600/15 rounded-full blur-3xl mix-blend-soft-light"></div>
      </div>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row items-center lg:gap-8 xl:gap-16 relative">
        <div className="w-full order-1 lg:order-2 space-y-8 sm:space-y-12 relative">
          <div className="space-y-4 sm:space-y-8 px-4 sm:px-0">
            <h2 className="text-3xl sm:text-4xl md:text-5xl xl:text-6xl leading-tight font-bold">
            ...
            <p className="text-base sm:text-lg md:text-xl leading-relaxed opacity-90 max-w-3xl font-serif italic border-1-4 bg-amber-500/60 pl-4 pl-2 bg-gradient-to-r from-white/5 to-transparent">
              "In our kitchen, passion meets precision. We craft not just meals,
              but culinary journeys that linger on the palate and in the
              heart.""
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 px-4 sm:px-0">
            {aboutfeature.map((item, i) => (
              <div
                key={i}
                className="flex flex-col items-center justify-center gap-3 sm:gap-4 transition-transform duration-300"
              >
                <div
                  className={`p-3 sm:p-4 rounded-full bg-gradient-to-r ${item.color} transition-transform duration-300 group-hover:scale-110`}
                >
        ...
        <div className="w-full order-2 lg:order-1 md:max-w-md lg:max-w-none lg:w-7/12 mt-12 mb-10 lg:mb-0 relative group transform hover:scale-[1.01] transition-all duration-500">
          ...
          <img
            src={AboutImage}
            alt="Restaurant"
            className="w-full auto object-cover aspect-[3/4] transform rotate-1 hover:rotate-0 transition-all duration-500 border-2 border-amber-600 bg-amber-400/10 rounded-[50px]"
          />
